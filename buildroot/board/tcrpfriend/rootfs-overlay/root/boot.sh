...
function history() {
	...
	0.1.5d Prevent duplicate TTYD/local-console boot.sh sessions from racing
	       ...
	       /tmp/tcrp-kexec.log.
	0.1.5e Static-IP boot now stops ConnMan before applying static addresses.
	       ConnMan previously obtained DHCP leases on the same ports after
	       setnetwork(), leaving a second address, duplicate DNS routes, and a
...
function setnetwork() {
    ...
    # ConnMan starts before boot.sh and independently DHCP-configures every
    # cable interface.  Stopping only dhcpcd/udhcpc was insufficient: ConnMan
    # could add a second DHCP address and per-DNS host routes after this
    # function had already configured a static port.  A static-IP session
    ...
    # DNS는 NIC별 값이 아니라 전역 하나다(2026-08-28). ConnMan/DHCP가
    # 남긴 nameserver와 127.0.0.1을 append 방식으로 보존하면 resolver
    # 경쟁이 다시 생기므로, static 모드에서는 netdns 값만 남긴다.
    staticdns="$(jq -r '.netdns.ipdns // empty' /mnt/tcrp/user_config.json 2>/dev/null)"
    if [ -n "${staticdns}" ]; then
        printf '# Generated by TCRP Friend static IP configuration\nnameserver %s\n' "${staticdns}" > /etc/resolv.conf
    else
        printf '# Generated by TCRP Friend static IP configuration\n' > /etc/resolv.conf
    fi

    local i primarydev="" primaryip="" primarygw=""
    for ((i = 0; i < count; i++)); do
    ...
    # Remove stale DHCP gateway host routes and add one explicit primary path
    # before installing the default route.
    if [ -n "${primarydev}" ] && [ -n "${primarygw}" ]; then
        while ip route del "${primarygw}" >/dev/null 2>&1; do :; done
        ip route replace "${primarygw}" dev "${primarydev}" | tee -a boot.log
        ip route replace default via "${primarygw}" dev "${primarydev}" | tee -a boot.log
        IP="${primaryip}"
    fi
}
